...
const ModalShipping = ({ props }) => {
  ...
  const formStateOnChange = async (value, onChange) => {
    ...
    await Woo.get(`/shipping-zones/${value}/locations`, {
      ...
      setState((previousState) => ({
        ...state,
        selectedState: previousState.selectedState === null ? value : previousState.selectedState,
        formCity: data,
        loadingCity: false,
      }));
    }).catch(err => {
  ...
  const formCityOnChange = (value, onChange) => {
    const _selectedCity = state.formCity.find(tmp => tmp.name == value);
    onChange(value);
    setState({
      ...state,
      selectedCity: value,
      metaCityCode: _selectedCity.code
    });
  }
    ...
    >
      <View style={[styles.modalChild]}>
          ...
          >
            ...
            <View style={{ flexDirection: "row", marginBottom: 16 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Controller
                  control={control}
                  render={({ onChange }) => state.loadingCity ? (
                    <View style={[styles.textInputWrapper]}>
                      <Text style={{fontFamily: "Roboto-Regular", color: "#ddd"}}>Loading...</Text>
                      </View>
                  ) :(
                    <View style={[styles.pickerWrapper]}>
                      <Picker note selectedValue={state.selectedCity} onValueChange={(value) => formCityOnChange(value, onChange)}>
                        <Picker.Item
                          label={
                            state.selectedState
                              ? "Select Region"
        ...
        <View style={{ flex: 1, justifyContent: "center" }}>
          <ModalButtons onCancel={closeModal} onSave={handleSubmit(_onSubmit)} />
        </View>
      </View>
    </Modal>
  );
